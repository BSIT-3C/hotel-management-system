      ...
      <div class="row ms-4 py-3 mt-3">
        ...
        <div class="shadow-sm rounded col-5" style="background-color: #d1d9f3;">
          ...
          <div class="text-center mt-3 mb-3">
            <img class="rounded-5 shadow-sm" src="{{$list->photo ? asset('storage/' . $list->photo) : asset('images/logo.png')}}" alt="" style="object-fit: cover; height: 297px; width: 250px;">
          </div>

          <div class="text-center" style="color: #1840C4;">
            <h3>{{$list->first_name}} {{$list->last_name}}</h3>
            <h5 class="fw-light">ROLE: {{$list->role}} </h5>
            <h5 class="fw-light">ID:<span class="fw-bold">{{$list->id}}</span></h5>
          </div> 
                
          <p class="text-center fw-light" style="color:#1840C4; margin-top: 25%;">JOINED {{$list->created_at}}</p>
        </div> <!--LEFT CARD-->
        

        <div class="col-7"> <!--RIGHT CARD-->
          <div class="row mt-5 ms-3">
            <h2 class="fw-bold"> {{$list->first_name}}  {{$list->last_name}} </h2>

            <div col-12 style="color: #1840C4;">
              <i class="fa-solid fa-envelope fa-lg" style=""></i><span class="ms-2 fs-5"> {{$list->email}}</span>
              <i class="fa-solid fa-house fa-lg ms-4" style=""></i><span class="ms-2 fs-5"> {{$list->address}}</span>
            </div>
          </div>
